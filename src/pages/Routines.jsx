...
import { Plus, Trash2, CheckCircle, Circle, Clock, Tag, AlertTriangle } from 'lucide-react';

const Routines = () => {
  const { routines, loading, addRoutine, removeRoutine, toggleRoutine, updateProgress } = useRoutines();
  ...
  const handleCreate = async (e) => {
    ...
    await addRoutine({ title, description, category, frequency, timeOfDay, priority });
  ...
  const handleProgressChange = async (id, progress) => {
    await updateProgress(id, progress, todayStr);
  };
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                          ...
                          <span className="badge" style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            {routine.frequency === 'Once' ? 'Today Only' : routine.frequency}
                          </span>
                          <span className="badge" style={{ 
                            background: routine.priority === 'High' ? 'rgba(239, 68, 68, 0.1)' : routine.priority === 'Medium' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)', 
                            color: routine.priority === 'High' ? '#ef4444' : routine.priority === 'Medium' ? '#f59e0b' : '#10b981', 
                            display: 'flex', alignItems: 'center', gap: '4px' 
                          }}>
                            <AlertTriangle size={12} /> {routine.priority} Priority
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      <span>Today's Progress: {currentProgress}%</span>
                    ...
                    <input 
                      ...
                      onChange={(e) => handleProgressChange(routine._id, parseInt(e.target.value))} 
                ...
                <div className="grid grid-cols-2" style={{ gap: '16px' }}>
                  <div className="form-group">
                    ...
                    <select className="form-control" value={frequency} onChange={e => setFrequency(e.target.value)}>
                      {['Daily', 'Weekly', 'Monthly', 'Once'].map(c => <option key={c} value={c} style={{ background: '#0f172a' }}>{c === 'Once' ? 'Once (Today Only)' : c}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
                    <label>Priority</label>
                    <select className="form-control" value={priority} onChange={e => setPriority(e.target.value)}>
                      {['High', 'Medium', 'Low'].map(c => <option key={c} value={c} style={{ background: '#0f172a' }}>{c}</option>)}
                    </select>
                  </div>
                </div>
